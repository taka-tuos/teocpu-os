...
extern FILE* __open(char *path, char *mode);

struct File_methods
{
    size_t (*write)(FILE* instance, const char *bp, size_t n);
    size_t (*read)(FILE* instance, char *bp, size_t n);
    int (*close)(FILE* instance);
    int (*seek)(FILE* instance, size_t offset, int base);
    long (*tell)(FILE* instance);
    int (*eof)(FILE* instance);
};
...
__extern_inline FILE *fopen(const char *filename, const char *mode)
{
    return __open(filename, mode);
}

__extern_inline int fclose(FILE *stream)
{
    if (stream->vmt->close == NULL) return EOF;
    return stream->vmt->close(stream);
}

__extern_inline int fseek(FILE *stream, size_t ofs, int base)
{
    if (stream->vmt->seek == NULL) return -1;
    return stream->vmt->seek(stream, ofs, base);
}

__extern_inline long ftell(FILE *stream)
{
    if (stream->vmt->tell == NULL) return -1;
    return stream->vmt->tell(stream);
}

__extern_inline int feof(FILE *stream)
{
    if (stream->vmt->eof == NULL) return -1;
    return stream->vmt->eof(stream);
}
